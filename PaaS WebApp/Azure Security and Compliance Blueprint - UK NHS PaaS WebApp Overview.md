...
For enhanced security, all resources in this solution are managed through Azure Resource Manager. Azure Active Directory enables both Azure Resource Manager and Azure Key Vault to enforce role-based access control to resources deployed in this solution. System health is monitored through Azure Security Center and Azure Monitor. Customers configure both monitoring services to capture logs and display system health in a single, easily navigable dashboard. Azure Application Gateway is configured as a firewall in prevention mode and disallows traffic that is not TLSv1.2. The solution utilizes Azure Application Service Environment v2 to isolate the web tier in a non-multi-tenant environment.
